...
from db import db, Attempt, QuestionAttempt, Question, Choice, Course, User
...
learning_bp = Blueprint('learning_assessment', __name__)

def randomize_question_choice_mapping(original_mapping):
    randomized_mapping = []
    questions = list(original_mapping.keys())
    random.shuffle(questions)

    for question in questions:
        choices = list(original_mapping[question].items())
        random.shuffle(choices)
        randomized_mapping.append((question, dict(choices)))

    return dict(randomized_mapping)

@learning_bp.route('/get_learning_assessment_questions', methods=['GET'])
def get_learning_assessment_questions():
    ...
    test = randomize_question_choice_mapping(question_choice_mapping)
    print(test, len(test))
    return jsonify(test), 200


@learning_bp.route('/submit_learning_assessment', methods=['POST'])
@auth_required
def determine_learning_styles(current_user):

    user_answers = request.json
    # Define the learning styles and initialize scores
    learning_styles = ["Collaborative", "Independent", "Experiential", "Dependent", "Adaptive"]
    scores = {style: 0 for style in learning_styles}

    # Assign scores based on user answers
    for question, answer in user_answers.items():
        # Use your own scoring logic here based on the mappings
        answer = answer.title()
        if answer == "Collaborative":
            scores["Collaborative"] += 1
        elif answer == "Independent":
            scores["Independent"] += 1
        elif answer == "Experiential":
            scores["Experiential"] += 1
        elif answer == "Dependent":
            scores["Dependent"] += 1
        elif answer == "Adaptive":
            scores["Adaptive"] += 1

    # Determine the primary and secondary learning styles
    primary_style = max(scores, key=scores.get)
    scores.pop(primary_style)

    if primary_style == "Collaborative" or primary_style == "Dependent":
        scores.pop("Independent")
    elif primary_style == "Independent":
        scores.pop("Collaborative")
        scores.pop("Dependent")
    
    secondary_style = max(scores, key=scores.get)
    user = User.query.filter_by(user_id=current_user.user_id).first()
    user.primary_learning_pattern = primary_style
    user.secondary_learning_pattern = secondary_style

    db.session.commit()
    return jsonify(primary_style, secondary_style), 200
